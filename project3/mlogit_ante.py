...
def loglike(theta, y, x):
    ...
    # denominator 
    denom = np.sum(np.exp(v), axis=1) # Fill in, dont keep dimensions -> WHY
    ...
    for i in range(N):
        v_i[i]= v[i, y[i]] # Fill in evaluate v at cols indicated by y 
...
def sim_data(theta, N: int):
    ...
    beta = np.hstack((zeros, theta)) # should be a (K,J) matrix: first column = zeros, remainder = theta 
